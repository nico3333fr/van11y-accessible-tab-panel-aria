...
/*  const triggerEvent = (el, event_type) => {
  ...
  const selectLink = (el) => {
        ...
        setTimeout(function(){ history.pushState(null, null, location.pathname + location.search + '#' + el.getAttribute(ATTR_CONTROLS))}, DELAY_HASH_UPDATE);
        }
  ...
  $listTabs()
    .forEach((tabs_node, index) => {
      ...
      // a
      $tabListLinks.forEach((tabListLink, index) => {
